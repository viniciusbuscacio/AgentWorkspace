...
func TestServesOverTLS(t *testing.T) {
	server, err := NewServer(&fakeBackend{}, Config{Addr: "127.0.0.1:0", Token: "secret-token", Version: "test", TLS: loopbackTLSConfig(t)})
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
	...
	// The resource-metadata endpoint needs no bearer; reach it over HTTPS.
	...
	resp, err := client.Get("https://" + server.Addr() + "/.well-known/oauth-protected-resource")
	if err != nil {
		t.Fatalf("HTTPS request failed: %v", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		t.Errorf("HTTPS metadata = %d, want 200", resp.StatusCode)
	}
	...
	if presp, perr := http.Get("http://" + server.Addr() + "/.well-known/oauth-protected-resource"); perr == nil {
